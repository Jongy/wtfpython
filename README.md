...
- The differences in the output of `gen_1` and `gen_2` in the second part is due the way variables `array_1` and `array_2` are re-assigned values.
...
- In the second case, the slice assignment to `array_2` updates the same old object `[1,2,3,4]` to `[1,2,3,4,5]`. Hence both `gen_2` and `array_2` still have reference to the same object (which has now been updated to `[1,2,3,4,5]`).
...
### ▶ Who am I?
<!-- Example ID: 219f276a-41af-abc8-b50f-6ed84ab87a18 --->
1. basic example
2. basic example with an attributeerror
3. slots
4. bases

#### 💡 Explanation:

* Each instance object holds a reference to its class (type). This reference is accessible via `__class__`, and it is writable.
* Instances are just attribute dictionaries with an owning class! Methods
* Slots?
* Don't try this at home :) There are few legitimate use cases. https://stackoverflow.com/questions/13280680/how-dangerous-is-setting-self-class-to-something-else
